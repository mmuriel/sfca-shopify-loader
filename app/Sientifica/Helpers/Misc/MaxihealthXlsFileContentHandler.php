...
class MaxihealthXlsFileContentHandler extends \Illuminate\Support\Collection implements IFileContentHandler {
	...
	public function processFile ($fileContent){
		...
		foreach ($arrContent as $index=>$record){
			...
			$recordDataRaw = preg_split("/__\-__/",$record);


			if (count($recordDataRaw) == 17){
					$productRaw->title = $recordDataRaw[1] != '""' ?  $recordDataRaw[1] : '';
					$productRaw->body_html = $recordDataRaw[5] != '""' ?  $recordDataRaw[5] : '';
					$productRaw->vendor = 'Maxihealth';
					$recordDataRaw[16] = $recordDataRaw[16] != '""' ?  $recordDataRaw[16] : '';
					$productRaw->multioption = $this->isMultiOptionVariant($recordDataRaw[16]);
		
					$recordDataRaw[14] = $recordDataRaw[14] != '""' ?  $recordDataRaw[14] : '';
					...
					$productRaw->metafields = [
						[
							"key" => 'directions',
							"value" => $recordDataRaw[10] != '""' ?  $recordDataRaw[10] : '' ,
							"type" => 'single_line_text_field',
							"namespace" => 'custom',
						],
						[
							"key" => 'short_description',
							"value" => $recordDataRaw[4] != '""' ?  $recordDataRaw[4] : '' ,
							"type" => 'single_line_text_field',
							"namespace" => 'custom',
						],
						[
							"key" => 'subtitle',
							"value" => $recordDataRaw[2] != '""' ?  $recordDataRaw[2] : '' ,
							"type" => 'single_line_text_field',
							"namespace" => 'descriptors',
						],
						[
							"key" => 'feature',
							"value" => $recordDataRaw[14] != '""' ?  $recordDataRaw[14] : '' ,
							"type" => 'single_line_text_field',
							"namespace" => 'custom',
						],
						[
							"key" => 'other_ingredients',
							"value" => $recordDataRaw[12] != '""' ?  $recordDataRaw[12] : '' ,
							"type" => 'multi_line_text_field',
							"namespace" => 'custom',
						],
						[
							"key" => 'suplement_facts',
							"value" => $recordDataRaw[11] != '""' ?  $recordDataRaw[11] : '' ,
							"type" => 'multi_line_text_field',
							"namespace" => 'custom',
						],
					];

					$recordDataRaw[6] != '""' ?  $recordDataRaw[6] : ''; 
					$recordDataRaw[7] != '""' ?  $recordDataRaw[7] : '';
					$recordDataRaw[8] != '""' ?  $recordDataRaw[8] : '';
					$recordDataRaw[9] != '""' ?  $recordDataRaw[9] : '';
